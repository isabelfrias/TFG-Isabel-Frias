/*  
 * CGRA version of the FFT algorithm
 * In this version, the system bus is configured in onetoM format, leading to stalls in the CGRA execution due to simultaneous memory accesses.
 * Futhermore, all loops are executed in the host, which results in multiple kernel launches to the CGRA and continuous reconfiguration of write and read pointers in each iteration. 
 * This provokes additional overhead due to the latency of launching kernels and waiting for interrupts for each butterfly block, in consequence, overall performance is reduced.
 */
...
int main(void) {
  printf("=== CGRA (without optimization) ===\n");
  ...
  // CGRA initialization
  cgra_t cgra;
  cgra.base_addr = mmio_region_from_addr((uintptr_t)CGRA_PERIPH_START_ADDRESS); // Set CGRA base address for memory-mapped registers

  cgra_wait_ready(&cgra);
  // Enable CGRA performance counters
  ...
  * 
  ...
  // Reset CGRA performance counters before starting FFT computation--------
  cgra_perf_cnt_reset(&cgra);
  CSR_READ(CSR_REG_MCYCLE, &fft_cycles_start);

  // Buffers for one butterfly
  static int32_t in_col0[2] __attribute__((aligned(4)));
  static int32_t in_col1[1] __attribute__((aligned(4)));
  static int32_t in_col2[2] __attribute__((aligned(4)));
  static int32_t in_col3[1] __attribute__((aligned(4)));

  static int32_t out_col0[1] __attribute__((aligned(4)));
  static int32_t out_col1[1] __attribute__((aligned(4)));
  static int32_t out_col2[1] __attribute__((aligned(4)));
  static int32_t out_col3[1] __attribute__((aligned(4)));

 // Loop that launches butterfly stages (based on the paper "Efficient and Flexible Implementation of FFT Application for CGRA Processor")
  for (int i = 0; i < numBits; i++) {
    int L = 1 << i;
    for (int j = 0; j < FFT_SIZE; j += 2 * L){
      for(int k = 0; k < L; k++){
        int index1 = j + k;
        int index2 = j + k + L;
        int tw_idx = FFT_SIZE * k / 2 / L;

        in_col0[0] = (fxp) in_X_r[index2];  
        in_col0[1] = (fxp) in_X_r[index1];

        in_col1[0] = (fxp) in_X_i[index2];

        in_col2[0] = (fxp) f_imag[tw_idx];  
        in_col2[1] = (fxp) in_X_i[index1];

        in_col3[0] = (fxp) f_real[tw_idx];

        cgra_wait_ready(&cgra);
        // Set CGRA input and output pointers for the current butterfly
        ...
        in_X_r[index1] = (fxp)out_col1[0];
        in_X_i[index1] = (fxp)out_col3[0];
        in_X_r[index2] = (fxp)out_col0[0];
        in_X_i[index2] = (fxp)out_col2[0];
      }
    }
  }
